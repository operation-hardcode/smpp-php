    ...
    public function testConnectionSuccess(): void
    {
        Amp\Loop::run(function (): \Generator {
            $connector = Connector::connect(function (): Amp\Promise {
                $bytes = new \SplQueue();
                $bytes->enqueue((string) (new BindReceiverResp('3333', CommandStatus::ESME_ROK()))->withSequence(1));
    ...
    public function testMessageWasConsumed(): void
    {
        Amp\Loop::run(function (): \Generator {
            $connector = Connector::connect(function (): Amp\Promise {
                $bytes = new \SplQueue();
                $bytes->enqueue((string) (new BindReceiverResp('3333', CommandStatus::ESME_ROK()))->withSequence(1));
                $bytes->enqueue((string) (new DeliverSm(new Destination('0001'), new Destination('1000'), new Utf8Message('Hello from test.')))->withSequence(2));
